...
def cipher(str, delta)
  ...
  ref = {}
  ("a".."z").each_with_object(ref) {|c, h| h[c] = ("a".ord + (c[0].ord - "a".ord + delta) % 26).chr }
  ("A".."Z").each_with_object(ref) {|c, h| h[c] = ("A".ord + (c[0].ord - "A".ord + delta) % 26).chr }
...
def test(id, str, delta, expected)
  puts "Test #{id}: #{cipher(str, delta) == expected ? "PASS" : "FAIL"} cipher('#{str}', #{delta}) == '#{expected}'"  
end

t = 0
test(t += 1, "abc", 1, "bcd");
test(t += 1, "abc1", 1, "bcd1");
test(t += 1, "wxYz", 3, "zaBc");
test(t += 1, "wx Yz!", 3, "za Bc!");
test(t += 1, "What a string!", 5, "Bmfy f xywnsl!");
